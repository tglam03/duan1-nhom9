...
if (!function_exists('loadAllofOneDay')) {
    function loadAllofOneDay($day, $dk)
    {
        ...
        try {
            $sql = "SELECT a.id as idorder,SUM(b.quantity) AS total_quantity,total_bill FROM orders a join oder_items b on a.id = b.order_id WHERE a.trangthai = 1 ";
            $sql .= " AND updated_at <= :ngaybd AND updated_at >= :ngaykthuc AND $dk GROUP BY a.id ORDER BY a.id DESC";
...
if (!function_exists('loadAllofdayProduct')) {
    function loadAllofdayProduct($day, $dk, $dkorderby)
    {
        ...
        $ngaybd = date('Y/m/d H:i:s');
        try {
            $sql = "SELECT hinh,ten_hh,b.product_id, SUM(b.quantity) AS total_quantity, SUM(b.quantity*b.price*(100-c.giam_gia)/100) AS tienthu FROM orders a join oder_items b on a.id = b.order_id join sanpham c on b.product_id = c.id WHERE a.trangthai = 1 ";
